...
ISI = 3 - interFrameInterval/6; %(substract interFrameInterval/3 to make sure that flipping is done at 3sec straight and not 1 frame later)
...
S1paStruct = struct; S1paStruct = buildFramesStruct(Win, S1paStruct, nFrames, frameDuration, allFrameNames(:,1), framePath);
S1piStruct = struct; S1piStruct = buildFramesStruct(Win, S1piStruct, nFrames, frameDuration, allFrameNames(:,2), framePath);
S1peStruct = struct; S1peStruct = buildFramesStruct(Win, S1peStruct, nFrames, frameDuration, allFrameNames(:,3), framePath);

S1faStruct = struct; S1faStruct = buildFramesStruct(Win, S1faStruct, nFrames, frameDuration, allFrameNames(:,4), framePath);
S1fiStruct = struct; S1fiStruct = buildFramesStruct(Win, S1fiStruct, nFrames, frameDuration, allFrameNames(:,5), framePath);
S1feStruct = struct; S1feStruct = buildFramesStruct(Win, S1feStruct, nFrames, frameDuration, allFrameNames(:,6), framePath);

S1laStruct = struct; S1laStruct = buildFramesStruct(Win, S1laStruct, nFrames, frameDuration, allFrameNames(:,7), framePath);
S1liStruct = struct; S1liStruct = buildFramesStruct(Win, S1liStruct, nFrames, frameDuration, allFrameNames(:,8), framePath);
S1leStruct = struct; S1leStruct = buildFramesStruct(Win, S1leStruct, nFrames, frameDuration, allFrameNames(:,9), framePath);


S2paStruct = struct; S2paStruct = buildFramesStruct(Win, S2paStruct, nFrames, frameDuration, allFrameNames(:,10), framePath);
S2piStruct = struct; S2piStruct = buildFramesStruct(Win, S2piStruct, nFrames, frameDuration, allFrameNames(:,11), framePath);
S2peStruct = struct; S2peStruct = buildFramesStruct(Win, S2peStruct, nFrames, frameDuration, allFrameNames(:,12), framePath);

S2faStruct = struct; S2faStruct = buildFramesStruct(Win, S2faStruct, nFrames, frameDuration, allFrameNames(:,13), framePath);
S2fiStruct = struct; S2fiStruct = buildFramesStruct(Win, S2fiStruct, nFrames, frameDuration, allFrameNames(:,14), framePath);
S2feStruct = struct; S2feStruct = buildFramesStruct(Win, S2feStruct, nFrames, frameDuration, allFrameNames(:,15), framePath);

S2laStruct = struct; S2laStruct = buildFramesStruct(Win, S2laStruct, nFrames, frameDuration, allFrameNames(:,16), framePath);
S2liStruct = struct; S2liStruct = buildFramesStruct(Win, S2liStruct, nFrames, frameDuration, allFrameNames(:,17), framePath);
S2leStruct = struct; S2leStruct = buildFramesStruct(Win, S2leStruct, nFrames, frameDuration, allFrameNames(:,18), framePath);


S3paStruct = struct; S3paStruct = buildFramesStruct(Win, S3paStruct, nFrames, frameDuration, allFrameNames(:,19), framePath);
S3piStruct = struct; S3piStruct = buildFramesStruct(Win, S3piStruct, nFrames, frameDuration, allFrameNames(:,20), framePath);
S3peStruct = struct; S3peStruct = buildFramesStruct(Win, S3peStruct, nFrames, frameDuration, allFrameNames(:,21), framePath);

S3faStruct = struct; S3faStruct = buildFramesStruct(Win, S3faStruct, nFrames, frameDuration, allFrameNames(:,22), framePath);
S3fiStruct = struct; S3fiStruct = buildFramesStruct(Win, S3fiStruct, nFrames, frameDuration, allFrameNames(:,23), framePath);
S3feStruct = struct; S3feStruct = buildFramesStruct(Win, S3feStruct, nFrames, frameDuration, allFrameNames(:,24), framePath);

S3laStruct = struct; S3laStruct = buildFramesStruct(Win, S3laStruct, nFrames, frameDuration, allFrameNames(:,25), framePath);
S3liStruct = struct; S3liStruct = buildFramesStruct(Win, S3liStruct, nFrames, frameDuration, allFrameNames(:,26), framePath);
S3leStruct = struct; S3leStruct = buildFramesStruct(Win, S3leStruct, nFrames, frameDuration, allFrameNames(:,27), framePath);


% put them all together
myVidStructArray = {S1paStruct, S1piStruct, S1peStruct, S1faStruct, S1fiStruct, S1feStruct, S1laStruct, S1liStruct, S1leStruct, S2paStruct, S2piStruct, S2peStruct, S2faStruct, S2fiStruct, S2feStruct, S2laStruct, S2liStruct, S2leStruct, S3paStruct, S3piStruct, S3peStruct, S3faStruct, S3fiStruct, S3feStruct, S3laStruct, S3liStruct, S3leStruct};
...
%%%pas nécessaire pour mon design
...
%     end
    ...
    for block = 1:nBlocks
        ...
        DrawFormattedText(Win, 'TÂCHE\n Appuyez quand une syllabe est répétée deux fois d''affilée', 'center', 'center', textColor);
        Screen('Flip', Win);
        ...
        elseif blockModality == auditoryCond
            ...
            dataFileName = [cd '/data/subj' num2str(subjNumber) '_ses-0' num2str(sesNumber) '_task-' expName '_rep-' num2str(rep) '_block-' num2str(block) '_' modality '_events.tsv'];
        ...
        pseudoRandExpTrialsBack = pseudorandExpTrials;
             for b=1:(length(stimSyll)+r)
                if r == 2
                    ...
                    elseif b == backTrials(1)+1 % this trial will be a target (repetition of the previous syllable - different actor)                         
...
%                 end
        ...
        for trial = 1:(nTrials+r)
            ...
            KbQueue('flush');
            KbQueue('start', {'s', 'd', 'space'});    %keep 's' in the allowed keys ???? OR WILL MESS UP WITH THE TRIGGER ?? 
            ...
            if blockModality == visualCond
                ...
                disp(strcat('Timing trial  ', num2str(trial), '- the duration was :', num2str(stimEnd-stimStart), ' sec')); %fb about duration in cw 
...
% %             end 
            ...
            disp(strcat('Timing trial ', num2str(trial), '- duration was :', num2str(stimEnd-stimStart), ' sec')); %fb about duration in cw 
            disp(strcat('Timing ISI - duration was :', num2str(ISIend-stimEnd), ' sec')); %fb about duration in cw 
                    ...
                    fprintf(fid, '%.3f\t%.3f\t%d\t%s\t%d\t%s\t%d\t%s\t%s\t%s\t%d\t%d\t%s\t%.3f\n', stimStart-expStart, stimEnd-stimStart, trial, pseudoRandExpTrialsBack(trial).stimulusname, block, modality, rep, pseudoRandExpTrialsBack(trial).actor, pseudoRandExpTrialsBack(trial).syllable(1), pseudoRandExpTrialsBack(trial).syllable(2), pseudoRandExpTrialsBack(trial).trialtype, howManyPress, keyName, keyTime);
                    fclose(fid);
